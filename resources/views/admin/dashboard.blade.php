                ...
                <div class="row">
                                        ...
                                        <ul class="list-group">
                                            @foreach($months as $i => $m)
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    {{ $m }}
                                                    <span class="badge text-bg-primary">{{ $bookingsMonthly[$i] }}</span>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                                    <div class="col-md-6">
                                        <h6>Revenue per Month</h6>
                                        <ul class="list-group">
                                            @foreach($months as $i => $m)
                                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                                    {{ $m }}
                                                    <span class="badge text-bg-success">{{ number_format($revenueMonthly[$i], 2) }}</span>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                                    ...
                                    <tbody>
                                        @forelse($recentBookings as $b)
                                            <tr>
                                                ...
                                                <td>{{ optional($b->trip->route->startStation)->name }} → {{ optional($b->trip->route->endStation)->name }}</td>
                    ...
                    <div class="col-lg-5 connectedSortable">
                                ...
                                <ul class="list-group list-group-flush">
                                    @forelse($topRoutes as $r)
                                        <li class="list-group-item d-flex justify-content-between align-items-center bg-transparent text-white">
                                            {{ $r->name }}
                                            <span class="badge text-bg-light">{{ $r->total }}</span>
                                        </li>
                                    @empty
                                        <li class="list-group-item bg-transparent text-white">No data</li>
                                    @endforelse
                                </ul>
                            </div>
                            <div class="card-footer border-0">
                                <small>Most booked routes</small>
                            </div>
                        </div>
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                @forelse($bookingsByStatus as $status => $count)
                                    <span class="badge text-bg-{{ $statusMap[$status] ?? 'secondary' }} me-2">{{ ucfirst($status) }}: {{ $count }}</span>
                                @empty
                                    <span>No data</span>
                                @endforelse
                            </div>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body p-0">
                                <table class="table mb-0">
                                    <thead><tr><th>Vehicle</th><th>Route</th><th>Departure</th></tr></thead>
                                    <tbody>
                                        @forelse($upcomingTrips as $t)
                                            <tr>
                                                <td>{{ $t->vehicle->name ?? 'N/A' }}</td>
                                                <td>{{ optional($t->route->startStation)->name }} → {{ optional($t->route->endStation)->name }}</td>
                                                <td>{{ \Carbon\Carbon::parse($t->departure_time)->format('Y-m-d H:i') }}</td>
                                            </tr>
                                        @empty
                                            <tr><td colspan="3" class="text-center">No upcoming trips</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
