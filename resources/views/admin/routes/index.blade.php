                    ...
                    <div class="col-md-12">
                        <div class="card mb-4">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h3 class="card-title mb-0">Route Table</h3>
                                <a href="{{ route('routes.create') }}" class="btn btn-sm btn-primary">Create Route</a>
                            </div>
                            <div class="card-body">
                                    ...
                                    <tbody>
                                        @forelse ($routes as $index => $route)
                                            <tr class="align-middle">
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ $route->startStation->name ?? 'N/A' }}</td>
                                                <td>{{ $route->endStation->name ?? 'N/A' }}</td>
                                                ...
                                                <td>
                                                    ...
                                                    <form action="{{ route('routes.destroy', $route->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure?')">
                                                        @csrf
                                                        @method('DELETE')
